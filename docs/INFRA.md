...
| Authentication → Providers → Google | **Configurado (2026-09-23).** *Enable* + *Client ID* y *Client secret* del cliente OAuth de Google Cloud (proyecto "Gallardo Crowdfunding" → Google Auth Platform: app publicada, público externo, dominio autorizado `gallardcode.com`, política de privacidad `/privacidad`, sin logo para no requerir verificación; cliente tipo *Aplicación web* con URI de redirección `https://<ref>.supabase.co/auth/v1/callback`). Sin activar el interruptor *Enable*, `/auth/v1/authorize?provider=google` devuelve 400 |
